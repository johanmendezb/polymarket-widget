...
open_decisions: []
tests_status: GREEN_121_UNIT (T5.1 brought the total to 90; T5.2 added 31 more: k-sampling, retry, timeout, and hand-computed median-of-log-odds/IQR aggregation tests, all against a mocked transport)
...
| T0 + 8h | T5.2 done (E5, only dependency was T5.1). `src/ai` now has an injectable `AnthropicTransport`, so every test runs against a mocked client - no network, no key, ever. k=5 blind samples run genuinely concurrently; one failing sample never fails the batch; a schema violation retries exactly once then surfaces `AI_INVALID_OUTPUT`; `insufficient_evidence: true` parses and flows through as a normal successful sample, not an error. Aggregation is median-of-log-odds with IQR dispersion, hand-verified in tests including k=1, an outlier, and the 0.01/0.99 boundary. A hard timeout (`AI_TIMEOUT_MS`, default 45000) is enforced by racing every call against the shared deadline independently of whether the transport itself honours the abort signal, so a hung upstream call cannot hang the route even in a misbehaving-transport test. The anchored diagnostic (`runAnchoredDiagnostic`, k=1) is a structurally separate type from the blind result, so it cannot be confused with the displayed estimate. ADR-0005 corrected: `claude-opus-5` rejects `temperature` outright (sampling parameters were removed from the model after the task contract was written), so it is never sent; `thinking` is omitted so the model runs adaptive by default; web search uses `web_search_20250305`, the only tool version this SDK release's stable surface supports. 31 new unit tests (121 total), all green, zero network calls. |
